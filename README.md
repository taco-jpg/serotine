...
- **Plugins:** open **Inbox settings → Plugins** to enable optional first-party features after reviewing their permissions. Private Chat requires compatible support from both participants. AI Summary adds a local `/summarize` workflow: review the selected ordinary text and confirm before it is sent to Cloudflare AI. Results stay local unless you explicitly send them. See [plugin behavior, privacy, and deployment setup](docs/SIP-5-7-plugins.md).
...
- **Group chats:** create a named group and invite contacts. Invitees become members only after explicit acceptance; invitations expire after seven days. The creator manages membership and can terminally dissolve the group. Ordinary members can leave. Groups support up to twenty people.
...
- **Private chats:** enable the Private Chat plugin on both participants' browsers, then open Private chat, check support, and choose 5 minutes, 1 hour, or 24 hours. Compatible participants use that timer for new text messages, measured from sending. Private text has no saved draft, search result, link preview, pin, reply, or backup copy. Files, polls, and mentions are unavailable while private mode is on. Turning the mode off leaves existing private messages on their original timers. Disabling the plugin preserves existing cleanup and prevents private sends from silently becoming ordinary messages.
...
- **Editing and receipts:** edit your own text with an edited label. Direct chats distinguish sending, relay confirmation, delivery, and optional read receipts. Groups and communities show sender-local Pending/Sent/Failed state without durable per-person delivered/read receipts.
- **Shared files and links:** browse a conversation's attachments and links together. With encrypted file storage configured, direct chats, groups, and community text channels support files up to 1 GiB each. Files are encrypted in bounded 4 MiB parts, authenticated before opening or downloading, and retained only for delivery: direct/private-group files are reclaimed after verified durable receipt by the intended recipients, with a seven-day fallback expiry. Community files expire after 30 days. Files cached on this device remain readable after server cleanup; clearing that cache can make them unavailable. Keep independent copies of files you need. Without file storage, the legacy relay supports up to 50 MiB, with smaller limits for large groups. Images and uploaded GIFs display at a useful size; tap or click to open the larger viewer. Supported videos play inline with playback controls, including on phones. Audio has inline controls; unsupported formats retain a download option.
...
The SIP 8–18 browser suites are `test:sharing`, `test:profiles`, `test:design`, and `test:groups`. Run `test:direct` on a runner that can establish real WebRTC connections; a restricted runner cannot certify direct connectivity. See the [implementation guide](docs/SIP-8-18.md) for scope and deployment checks.
...
- A relay-confirmed event is not automatically a delivered or read message. Direct chats use separate recipient receipts; read receipts can be disabled. Group/community attachment-completion acknowledgements control storage cleanup only and do not create reader lists.
...
- Ordinary conversations use the retained encrypted event relay for device synchronization. **Force P2P** adds mutually agreed, direct-only encrypted text and files up to 2 MiB, with no relay or TURN fallback. Both participants must keep Serotine open and successfully connect.
