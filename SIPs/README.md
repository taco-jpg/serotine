# SIP Index

Serotine Improvement Proposals live in this directory.

| SIP | Title | Status |
| --- | --- | --- |
| [SIP-1](SIP-1-public-communities-and-channels.md) | Public communities and channels | Draft |
| [SIP-2](SIP-2-expanded-and-custom-themes.md) | Expanded and custom themes | Draft |
| [SIP-3](SIP-3-voice-and-video-calling.md) | Voice and video calling | Draft |
| [SIP-4](SIP-4-favicon-and-unread-badge.md) | Favicon and unread badge | Draft |
| [SIP-5](SIP-5-plugin-system.md) | Plugin system | Draft |
| [SIP-6](SIP-6-private-chat-plugin.md) | Private chat plugin | Draft |
| [SIP-7](SIP-7-ai-summary-plugin.md) | AI summary plugin | Draft |
| [SIP-8](SIP-8-share-selected-messages.md) | Share selected messages | Draft |
| [SIP-9](SIP-9-force-p2p-mode.md) | Force P2P mode | Draft |

These proposals are open for discussion. Draft status does not mean a feature has been accepted or implemented.

When adding one, use a filename like:

`SIP-10-example-title.md`

The repository root contains the process, format, status definitions, and template.
