...
export async function deleteMessage(messageId: string) {
  ...
  // Use admin client to bypass RLS for deletion, but verify permission first
  const adminSupabase = createAdminClient()

  // 1. Fetch the message to verify permission
  const { data: message, error: fetchError } = await adminSupabase
    .from("chat_messages")
    .select("sender_id, recipient_keys")
    .eq("id", messageId)
    .single()
  ...
  // 2. Check if user is sender OR a recipient
  const isSender = message.sender_id === user.id
  const isRecipient = message.recipient_keys && Object.keys(message.recipient_keys).includes(user.id)
  ...
  const { error: deleteError } = await adminSupabase.from("chat_messages").delete().eq("id", messageId)
