...
import { ArrowLeft, Check, CheckCheck, Loader2, Lock, Send, Shield, RotateCw, ArrowDown, Search, X, ChevronUp, ChevronDown, Copy, Reply, Pencil, Pin, PinOff, Info, Users, BarChart3, AtSign, Bell, Ban, ExternalLink, MessageSquare, MoreHorizontal, Trash2, Timer, KeyRound } from "lucide-react"
  ...
  return <div ref={chatRoot} className="relative flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden">
    ...
    <footer className="chat-composer chat-bottom min-w-0 shrink-0 overflow-y-auto border-t border-border/70 bg-background px-3 pt-2 sm:px-5">
      ...
      <div className="mx-auto max-w-4xl">
        ...
        {!privateMode && mentionOpen && <div id="message-mention-options" role="listbox" aria-label="Mention suggestions" className="mb-2 max-h-48 overflow-y-auto rounded-lg border border-border bg-card p-1 text-sm">
        ...
        {privateMode ? <div className="flex flex-wrap items-center justify-between gap-2 py-1"><Button type="button" variant="ghost" size="sm" disabled={!usable || sending} onClick={() => setSecretOpen(true)}><KeyRound className="size-4" />Share access key</Button><span className="text-[11px] text-muted-foreground">Plain text only · No files or polls</span></div> : <AttachmentComposer key={`${myPub}:${conversationId}`} owner={myPub} extraActions={<>
