...
import { ArrowLeft, Check, CheckCheck, Loader2, Lock, Send, Shield, RotateCw, ArrowDown, Search, X, ChevronUp, ChevronDown, Copy, Reply, Pencil, Pin, PinOff, Info, Users, BarChart3, AtSign, Bell, Ban, ExternalLink, MessageSquare, MoreHorizontal, Trash2 } from "lucide-react"
...
export default function ChatWindow({ params }: { params: { pubkey: string } }) {
  ...
  useEffect(() => {
    ...
    setBusy(false); setRetrying(null); setBatchProgress(null); setSendError(""); setReplyTo(undefined); setDeleteTarget(null); setDeleteError("")
  ...
  return <div ref={chatRoot} className="relative flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden">
    <header className="flex min-h-14 shrink-0 items-center justify-between gap-2 border-b border-border/80 bg-card/70 px-3 py-2 sm:px-5">
      <div className="flex min-w-0 items-center gap-3"><Link href="/chat" aria-label="Back to conversations" className="flex size-11 shrink-0 items-center justify-center rounded-lg text-muted-foreground hover:bg-card md:hidden"><ArrowLeft className="size-5" /></Link>{isGroup ? <span className="flex size-10 shrink-0 items-center justify-center rounded-xl bg-primary/10 text-primary"><Users className="size-5" /></span> : <span className="hidden shrink-0 sm:block"><IdentityIcon pubKey={conversationId} size={38} /></span>}<div className="min-w-0"><h1 className="truncate font-sans text-base font-medium">{title}{isSelf && <span className="ml-2 text-xs font-normal text-muted-foreground">yourself</span>}</h1><span className="mt-1 flex items-center gap-1.5 text-xs text-muted-foreground"><Lock className="size-3 shrink-0" /><span className="truncate">{conversation?.archived ? "Archived · History saved" : isGroup ? unavailableGroup ? "Group history unavailable" : closedGroup ? "Closed group · Encrypted" : leftGroup ? "Left group · Encrypted" : `${activeMembers.length} members · Encrypted` : "End-to-end encrypted"}</span></span><span role="status" className={`mt-1 block text-[11px] lg:hidden ${status === "offline" ? "text-amber-700 dark:text-amber-300" : "text-muted-foreground"}`}>{statusLabel}</span></div></div>
      <div className="flex shrink-0 items-center gap-1"><span role="status" className={`mr-2 hidden rounded-lg border px-2.5 py-1.5 text-xs lg:inline ${status === "offline" ? "border-amber-400/20 text-amber-700 dark:text-amber-300" : "border-border text-muted-foreground"}`}>{statusLabel}</span><Button variant="ghost" size="icon" aria-label="Search conversation" aria-expanded={searchOpen} onClick={() => { setSearchOpen(!searchOpen); setQuery(""); setMatchIndex(0) }}><Search className="size-4" /></Button><Button variant="ghost" size="icon" aria-label="Conversation details, files and settings" onClick={() => { setInfoTab("settings"); setInfoOpen(true) }}><Info className="size-4" /></Button>{conversation && !isSelf && <ConversationActions conversation={conversation} disabled={sending || actionBusy} onError={setSendError} onNotice={setAnnouncement} />}</div>
    </header>
    {searchOpen && <div role="search" aria-label="Search saved messages" className="flex flex-wrap items-center gap-2 border-b border-border px-4 py-3 sm:px-7"><Input ref={searchInput} aria-label="Search messages" placeholder="Search this conversation…" className="min-w-32 flex-1" value={query} onChange={event => { setQuery(event.target.value); setMatchIndex(0) }} onKeyDown={event => { if (event.nativeEvent.isComposing) return; if (event.key === "Escape") { setSearchOpen(false); setQuery(""); input.current?.focus() } else if (event.key === "Enter") { event.preventDefault(); moveMatch(event.shiftKey ? -1 : 1) } }} /><span role="status" className="text-xs text-muted-foreground">{searchTerm ? matches.length ? `${selectedIndex + 1} of ${matches.length}` : "No matches" : "Saved messages"}</span><Button variant="ghost" size="icon" aria-label="Previous match" disabled={!matches.length} onClick={() => moveMatch(-1)}><ChevronUp className="size-4" /></Button><Button variant="ghost" size="icon" aria-label="Next match" disabled={!matches.length} onClick={() => moveMatch(1)}><ChevronDown className="size-4" /></Button><Button variant="ghost" size="icon" aria-label="Close search" onClick={() => { setSearchOpen(false); setQuery(""); input.current?.focus() }}><X className="size-4" /></Button></div>}
    ...
    <div ref={messagesViewport} role="region" aria-label="Conversation messages" tabIndex={0} className="min-h-0 min-w-0 flex-1 overflow-x-hidden overflow-y-auto overscroll-contain px-3 py-3 sm:px-6" onScroll={event => { const node = event.currentTarget; const wasNearBottom = nearBottom.current; nearBottom.current = node.scrollHeight - node.scrollTop - node.clientHeight < 120; setAwayFromBottom(!nearBottom.current); if (nearBottom.current) { setUnseen(0); if (!wasNearBottom) acknowledgeVisible() } }}>
      <div className="mx-auto max-w-4xl space-y-1.5">{messages.length > 0 ? <p className="flex items-center justify-center gap-1.5 py-1 text-[11px] text-muted-foreground"><Lock className="size-3" />Messages are end-to-end encrypted.</p> : <div className="mx-auto max-w-sm py-7 text-center">{isSelf ? <MessageSquare className="mx-auto mb-3 size-8 text-primary/60" /> : <Shield className="mx-auto mb-3 size-8 text-primary/60" />}<h2 className="mb-2 text-xl text-foreground">{isSelf ? "Send things to yourself." : isGroup ? title : "Start with a hello."}</h2><p className="text-sm leading-relaxed text-muted-foreground">{isSelf ? "Send yourself messages, files, voice notes and links, just like any other conversation." : isGroup ? "A shared conversation with your group. Messages are encrypted before leaving your device." : "Verify this contact’s address through another trusted channel. Messages are encrypted before leaving your device."}</p>{!isGroup && <details className="mt-3 text-xs text-muted-foreground"><summary className="cursor-pointer">{isSelf ? "Your address" : "Contact address"}</summary><p className="mt-2 select-all break-all font-mono leading-relaxed">{conversationId}</p></details>}</div>}
        ...
        {messages.map((message, index) => {
          ...
          return <div key={message.id} id={`message-${message.id}`} ref={node => { if (node) messageNodes.current.set(message.id, node); else messageNodes.current.delete(message.id) }} className={activeMatch === message.id || highlightedId === message.id ? "rounded-xl ring-2 ring-amber-300/60 ring-offset-4 ring-offset-background" : ""}>{previousDay !== new Date(message.timestamp).toDateString() && <p className="mb-2 pt-1 text-center text-[11px] text-muted-foreground">{day}</p>}<div className={`flex flex-col ${mine ? "items-end" : "items-start"}`}>
            {isGroup && !mine && <p className="mb-0.5 ml-1 text-xs text-primary">{displayName(message.senderPubKey)}</p>}<span className="sr-only">{displayName(message.senderPubKey)}:</span>
            <div className={`flex w-full min-w-0 items-start gap-1 ${mine ? "flex-row-reverse" : ""}`}>
              <div className={`min-w-0 max-w-[calc(100%-3rem)] break-words rounded-xl px-2.5 py-1.5 text-[15px] leading-[1.4] [overflow-wrap:anywhere] sm:max-w-[85%] ${mine ? "rounded-br-md bg-message-outgoing text-message-outgoing-foreground" : "rounded-bl-md border border-border/70 bg-message-incoming text-message-incoming-foreground"}`}>
                {message.replyTo && <button type="button" disabled={!quoted} aria-label={quoted ? "Jump to replied message" : "Original message is unavailable"} className="mb-1.5 block w-full rounded-r border-l-2 border-current/40 bg-current/5 px-2 py-1 text-left text-xs" onClick={() => { if (quoted) jumpToMessage(quoted.id) }}><span className="block font-medium">{quoted ? displayName(quoted.senderPubKey) : "Reply"}</span><span className="line-clamp-2 opacity-70">{quoted ? messageSummary(quoted) : "Original message is not saved on this device."}</span></button>}
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent align={mine ? "end" : "start"} onCloseAutoFocus={event => {
                  ...
                  <DropdownMenuItem className="min-h-11" onSelect={async () => { try { await navigator.clipboard.writeText(messageSummary(message)); clearTimeout(copyTimer.current); setCopiedMessage(message.id); setAnnouncement("Message copied."); copyTimer.current = setTimeout(() => setCopiedMessage(""), 2500) } catch { setSendError("Clipboard access was blocked. Select the message text to copy it.") } }}>{copiedMessage === message.id ? <Check /> : <Copy />}{copiedMessage === message.id ? "Copied" : "Copy"}</DropdownMenuItem>
                  <DropdownMenuItem className="min-h-11" disabled={!usable} onSelect={() => { afterMessageMenuClose.current = () => { setReplyTo(message.id); input.current?.focus() } }}><Reply />Reply</DropdownMenuItem>
                  {mine && !!message.content && !message.attachment && !message.poll && <DropdownMenuItem className="min-h-11" disabled={!usable || actionBusy} onSelect={() => { afterMessageMenuClose.current = () => setEditing(message) }}><Pencil />Edit</DropdownMenuItem>}
                  <DropdownMenuItem className="min-h-11" disabled={!usable || actionBusy} onSelect={() => void act(() => messaging.pinMessage(conversationId, message.id, !message.pinned), message.pinned ? "Message unpinned." : "Message pinned.")}>{message.pinned ? <PinOff /> : <Pin />}{message.pinned ? "Unpin" : "Pin"}</DropdownMenuItem>
            ...
            <div className="mt-0.5 flex max-w-full flex-wrap items-center gap-x-1.5 gap-y-0.5 text-[11px] leading-4 text-muted-foreground"><time title={new Date(message.timestamp).toLocaleString()} dateTime={new Date(message.timestamp).toISOString()}>{new Date(message.timestamp).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}</time>{message.editedAt && <span title={`Edited ${new Date(message.editedAt).toLocaleString()}`}>edited</span>}{message.pinned && <Pin aria-label="Pinned message" className="size-3 text-primary" />}
            ...
            {mine && message.delivery === "failed" && message.error && message.error !== deliveryError && message.error !== sendError && <p className="mt-1 max-w-[94%] text-xs text-destructive [overflow-wrap:anywhere] sm:max-w-[85%]">{message.error}</p>}
          </div></div>
        })}
      </div>
    </div>
    ...
    <footer className="chat-composer chat-bottom min-w-0 shrink-0 overflow-y-auto border-t border-border/70 bg-background px-3 pt-2 sm:px-5">
      {ready && unavailableGroup && <p className="mx-auto mb-3 max-w-4xl text-sm text-muted-foreground">This group has no saved conversation on this device. A new group message may make it available again.</p>}
      {leftGroup && <p className="mx-auto mb-3 max-w-4xl text-sm text-muted-foreground">{closedGroup ? "This group is closed. Saved messages are still available." : "You are no longer a member of this group. Your saved conversation is still available."}</p>}
      {membershipUpdating && <p className="mx-auto mb-3 max-w-4xl text-sm text-muted-foreground">Updating group membership before sending more messages…</p>}
      {(unconfirmed.length > 1 || batchProgress) && <div className="mx-auto mb-3 flex max-w-4xl flex-wrap items-center justify-between gap-2 text-xs text-amber-700 dark:text-amber-200"><span role="status">{batchProgress ? `Retrying ${batchProgress.current} of ${batchProgress.total}…` : `${unconfirmed.length} messages need another attempt.`}</span><Button type="button" size="sm" variant="outline" disabled={!ready || sending} onClick={() => void retryFailed(unconfirmed, true)}><RotateCw className={`size-3 ${batchProgress ? "animate-spin" : ""}`} />{batchProgress ? "Retrying…" : "Retry failed messages"}</Button></div>}
      <div className="mx-auto max-w-4xl">
        {replyTo && <div className="mb-2 flex items-center gap-2 rounded-lg border-l-2 border-primary bg-card p-2 text-xs"><Reply className="size-4 shrink-0 text-primary" /><span className="min-w-0 flex-1"><span className="block text-primary">Replying to {byId.get(replyTo) ? displayName(byId.get(replyTo)!.senderPubKey) : "message"}</span><span className="block truncate text-muted-foreground">{byId.get(replyTo) ? messageSummary(byId.get(replyTo)!) : "Original message"}</span></span><Button variant="ghost" size="icon" aria-label="Cancel reply" onClick={() => setReplyTo(undefined)}><X className="size-4" /></Button></div>}
        <form onSubmit={event => { event.preventDefault(); void submit() }}><div className="flex items-end gap-2 rounded-xl border border-border bg-card p-1.5 shadow-sm focus-within:border-primary/60 focus-within:ring-2 focus-within:ring-primary/10"><Textarea ref={input} aria-label="Message" aria-busy={busy} placeholder={blocked ? "Unblock this person to send messages" : request ? "Accept this conversation to reply" : unavailableGroup ? "This group is unavailable on this device" : closedGroup ? "This group is closed" : leftGroup ? "You left this group" : membershipUpdating ? "Updating group membership…" : ready ? isSelf ? "Message yourself…" : "Write a message…" : "Opening conversation…"} value={content} onChange={event => changeMessage(event.target.value, event.target.selectionStart, event.target.selectionEnd)} onSelect={event => { const node = event.currentTarget; if (document.activeElement === node) updateMentionQuery(node.value, node.selectionStart, node.selectionEnd) }} onBlur={() => setMentionOpen(false)} aria-autocomplete="list" aria-controls={mentionOpen ? "message-mention-options" : undefined} aria-activedescendant={mentionOpen && mentionCandidates.length ? `message-mention-${selectedMentionIndex}` : undefined} maxLength={MAX_MESSAGE_LENGTH} disabled={!usable || !draftReady} readOnly={busy} rows={1} className="max-h-44 min-h-9 min-w-0 flex-1 resize-none border-0 bg-transparent text-base shadow-none focus-visible:ring-0" onKeyDown={event => {
        ...
        {mentionOpen && <div id="message-mention-options" role="listbox" aria-label="Mention suggestions" className="mb-2 max-h-48 overflow-y-auto rounded-lg border border-border bg-card p-1 text-sm">
        ...
        <AttachmentComposer key={`${myPub}:${conversationId}`} owner={myPub} extraActions={<>
          <div className="hidden items-center gap-1 sm:contents"><Button type="button" variant="ghost" size="sm" disabled={!usable} onClick={() => setPollOpen(true)}><BarChart3 className="size-4" />Poll</Button>{!isSelf && <Button type="button" variant="ghost" size="sm" disabled={!usable} aria-expanded={mentionOpen} onClick={() => { setMentionQuery(findMentionQuery(content, input.current?.selectionStart ?? content.length, input.current?.selectionEnd ?? content.length)); setMentionIndex(0); setMentionOpen(!mentionOpen) }}><AtSign className="size-4" />Mention</Button>}<span className="ml-auto hidden text-[11px] lg:inline text-muted-foreground">Math: $…$ · Code: ```</span></div>
          <DropdownMenu>
            <DropdownMenuTrigger asChild><Button type="button" variant="ghost" size="icon" className="sm:hidden" aria-label="More message tools" title="More message tools" disabled={!usable}><MoreHorizontal className="size-4" /></Button></DropdownMenuTrigger>
            <DropdownMenuContent align="end" onCloseAutoFocus={event => { const action = afterMessageMenuClose.current; if (action) { event.preventDefault(); afterMessageMenuClose.current = null; action() } }}>
              <DropdownMenuItem onSelect={() => { afterMessageMenuClose.current = () => setPollOpen(true) }}><BarChart3 />Create poll</DropdownMenuItem>
              {!isSelf && <DropdownMenuItem onSelect={() => { afterMessageMenuClose.current = () => { setMentionQuery(findMentionQuery(content, input.current?.selectionStart ?? content.length, input.current?.selectionEnd ?? content.length)); setMentionIndex(0); setMentionOpen(true); requestAnimationFrame(() => document.getElementById("message-mention-0")?.focus()) } }}><AtSign />Mention someone</DropdownMenuItem>}
            </DropdownMenuContent>
          </DropdownMenu>
    ...
    <Dialog open={infoOpen} onOpenChange={setInfoOpen}><DialogContent className="max-h-[90dvh] overflow-y-auto sm:max-w-xl"><DialogHeader><DialogTitle>{title}</DialogTitle><DialogDescription>Conversation settings and everything shared here.</DialogDescription></DialogHeader><div role="tablist" aria-label="Conversation details" className="grid grid-cols-4 gap-1 rounded-lg bg-card p-1">{([['settings', 'Settings'], ['files', 'Files'], ['links', 'Links'], ['pins', 'Pinned']] as const).map(([tab, label]) => <button key={tab} type="button" id={`details-tab-${tab}`} role="tab" aria-selected={infoTab === tab} tabIndex={infoTab === tab ? 0 : -1} aria-controls={`details-panel-${tab}`} onKeyDown={event => { const tabs = ["settings", "files", "links", "pins"] as const; const index = tabs.indexOf(tab); const next = event.key === "ArrowRight" ? tabs[(index + 1) % tabs.length] : event.key === "ArrowLeft" ? tabs[(index + tabs.length - 1) % tabs.length] : event.key === "Home" ? tabs[0] : event.key === "End" ? tabs[tabs.length - 1] : null; if (next) { event.preventDefault(); setInfoTab(next); document.getElementById(`details-tab-${next}`)?.focus() } }} onClick={() => setInfoTab(tab)} className={`rounded-md px-2 py-2 text-sm ${infoTab === tab ? "bg-muted text-foreground" : "text-muted-foreground hover:text-foreground"}`}>{label}</button>)}</div><div role="tabpanel" id={`details-panel-${infoTab}`} aria-labelledby={`details-tab-${infoTab}`} className="min-h-32 space-y-4">
      ...
      {infoTab === "settings" && <>
        ...
        {!isGroup && <div className="space-y-2"><p className="text-sm font-medium">{isSelf ? "Your address" : "Contact address"}</p><p className="select-all break-all rounded-lg bg-card p-3 font-mono text-xs text-muted-foreground">{conversationId}</p></div>}
