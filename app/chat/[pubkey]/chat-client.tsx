...
export default function ChatWindow({ params }: { params: { pubkey: string } }) {
  ...
  const mentionCandidates = (isGroup ? activeMembers : (isSelf ? [] : [conversationId])).filter(pub => pub !== myPub && !mentions.includes(pub))
  ...
  useEffect(() => {
    ...
    setBusy(false); setRetrying(null); setBatchProgress(null); setSendError(""); setReplyTo(undefined); setMentions([])
  ...
  const submit = async () => {
    ...
    try {
      ...
      clearSubmittedDraft()
      if (actionScope.current === scope) { setReplyTo(undefined); setMentions([]); setMentionOpen(false) }
  ...
  return <div className="flex min-h-0 flex-1 flex-col overflow-hidden">
    ...
    <footer className="max-h-[55dvh] shrink-0 overflow-y-auto border-t border-zinc-800/80 bg-zinc-950 p-3 sm:px-8 sm:py-4">
      ...
      <div className="mx-auto max-w-3xl">
        ...
        <form onSubmit={event => { event.preventDefault(); void submit() }}><div className="flex items-end gap-2 rounded-xl border border-zinc-700 bg-zinc-900 p-2 focus-within:border-indigo-300/60"><Textarea ref={input} aria-label="Message" aria-busy={busy} placeholder={blocked ? "Unblock this person to send messages" : request ? "Accept this conversation to reply" : closedGroup ? "This group is closed" : leftGroup ? "You left this group" : membershipUpdating ? "Updating group membership…" : ready ? isSelf ? "Send yourself a message…" : "Write a message…" : "Opening conversation…"} value={content} onChange={event => setContent(event.target.value)} maxLength={MAX_MESSAGE_LENGTH} disabled={!usable || !draftReady} readOnly={busy} rows={2} className="max-h-44 min-h-12 resize-none border-0 bg-transparent text-base shadow-none focus-visible:ring-0" onKeyDown={event => { if (event.key === "Enter" && !event.shiftKey && !event.nativeEvent.isComposing) { event.preventDefault(); void submit() } }} /><Button type="submit" aria-label="Send message" disabled={!usable || !draftReady || !content.trim() || sending} size="icon" className="mb-1 mr-1 shrink-0">{busy ? <Loader2 className="size-4 animate-spin" /> : <Send className="size-4" />}</Button></div></form>
        <div className="mt-2 flex flex-wrap items-center gap-1"><Button type="button" variant="ghost" size="sm" disabled={!usable} onClick={() => setPollOpen(true)}><BarChart3 className="size-4" />Poll</Button>{!isSelf && <Button type="button" variant="ghost" size="sm" disabled={!usable} aria-expanded={mentionOpen} onClick={() => setMentionOpen(!mentionOpen)}><AtSign className="size-4" />Mention</Button>}<span className="ml-auto text-[11px] text-zinc-500">Math: $…$ · Code: ```</span></div>
        {(mentionOpen || mentions.length > 0) && <div className="mb-2 flex flex-wrap items-center gap-2 rounded-lg border border-zinc-800 p-2 text-xs">{mentions.map(pub => <button type="button" key={pub} aria-label={`Remove mention of ${displayName(pub)}`} onClick={() => setMentions(current => current.filter(item => item !== pub))} className="inline-flex items-center gap-1 rounded-full bg-indigo-400/15 px-2 py-1 text-indigo-200">@{displayName(pub)}<X className="size-3" /></button>)}{mentionOpen && mentionCandidates.map(pub => <button type="button" key={pub} className="rounded px-2 py-1 text-zinc-300 hover:bg-zinc-800" onClick={() => { setMentions(current => [...current, pub]); setMentionOpen(false); input.current?.focus() }}>Mention {displayName(pub)}</button>)}{mentionOpen && mentionCandidates.length === 0 && <span className="text-zinc-500">Everyone is already mentioned.</span>}</div>}
        <AttachmentComposer disabled={!usable || sending} onSend={async (file, kind, onProgress) => { const scope = actionScope.current; const id = await sendAttachment(messaging.sendEvent, conversationId, file, kind, onProgress, replyTo); if (actionScope.current === scope) { setReplyTo(undefined); jumpToLatest() } return id }} />
