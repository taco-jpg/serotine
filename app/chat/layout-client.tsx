...
function InboxLayout({ children }: { children: React.ReactNode }) {
  ...
  return <div className="chat-shell flex min-h-0 overflow-hidden bg-background text-foreground">
    <aside id="serotine-sidebar" aria-label="Inbox" data-collapsed={sidebarCollapsed} className={`${inConversation ? "hidden md:flex" : "flex"} min-h-0 min-w-0 w-full flex-col overflow-hidden border-r border-border bg-sidebar text-sidebar-foreground transition-[width] duration-200 ease-out motion-reduce:transition-none md:shrink-0 ${sidebarCollapsed ? "md:w-16" : "md:w-[15.5rem]"}`}>
      <div className={`flex h-12 shrink-0 items-center gap-1 border-b border-border px-2 ${sidebarCollapsed ? "md:justify-center" : ""}`}>
        <button type="button" onClick={() => void copy(identity.publicKey)} title={`My identity · ${shortAddress(identity.publicKey)} · Copy public address`} aria-label="Copy your public address" className={`flex size-11 shrink-0 items-center justify-center rounded-lg hover:bg-accent focus-visible:outline-2 focus-visible:outline-ring md:size-8 ${sidebarCollapsed ? "md:hidden" : ""}`}>{copied ? <Check className="size-5 text-primary" /> : <IdentityIcon pubKey={identity.publicKey} size={28} />}</button>
        <Link href="/chat" className={`flex min-h-11 min-w-0 flex-1 items-center rounded-lg focus-visible:outline-2 focus-visible:outline-ring ${sidebarCollapsed ? "md:hidden" : ""}`}><span className="truncate text-base font-semibold tracking-tight text-foreground">Serotine</span></Link>
      ...
      <div className={`min-h-0 flex-1 overflow-y-auto overscroll-contain ${sidebarCollapsed ? "md:hidden" : ""}`}>
        ...
        <div className="mb-1 flex items-center gap-1 px-2 pt-1">
          ...
          <div className="flex min-w-0 flex-1 gap-0.5 rounded-lg bg-muted/70 p-0.5" role="group" aria-label="Conversation view">
            <button type="button" aria-pressed={conversationView === "inbox"} title={`${activeConversations.length} chats${totalUnread ? ` · ${totalUnread} unread` : ""}`} onClick={() => { setConversationView("inbox"); setFilter(""); setNotice("") }} className={`flex min-h-11 flex-1 items-center justify-center gap-1 rounded-md px-1.5 text-[11px] font-medium focus-visible:outline-2 focus-visible:outline-ring md:min-h-7 ${conversationView === "inbox" ? "bg-background text-foreground shadow-xs" : "text-muted-foreground hover:text-foreground"}`}>Inbox<span className="text-muted-foreground">{activeConversations.length}</span>{totalUnread > 0 && <span className="sr-only"> · {totalUnread} unread</span>}</button>
            <button type="button" aria-pressed={conversationView === "archived"} onClick={() => { setConversationView("archived"); setFilter(""); setNotice("") }} className={`flex min-h-11 flex-1 items-center justify-center gap-1 rounded-md px-1.5 text-[11px] font-medium focus-visible:outline-2 focus-visible:outline-ring md:min-h-7 ${conversationView === "archived" ? "bg-background text-foreground shadow-xs" : "text-muted-foreground hover:text-foreground"}`}>Archived<span className="text-muted-foreground">{archivedConversations.length}</span></button>
          </div>
        ...
        {(visibleConversations.length > 3 || filter) && <div className="relative mx-2 mb-1.5"><Search className="pointer-events-none absolute left-2.5 top-1/2 size-3.5 -translate-y-1/2 text-muted-foreground" /><Input aria-label="Filter conversations" placeholder="Filter conversations" value={filter} onChange={event => setFilter(event.target.value)} className="pl-8 md:h-8" /></div>}
        ...
        <nav className="px-2 pb-2" aria-label={conversationView === "archived" ? "Archived conversations" : "Conversations"}>
      ...
      <div className={`chat-bottom shrink-0 border-t border-border bg-sidebar px-3 py-2 text-muted-foreground ${sidebarCollapsed ? "md:px-2" : ""}`}>
        <div className={`flex items-center justify-between gap-1 ${sidebarCollapsed ? "md:flex-col" : ""}`}>
          <span className={`flex min-w-0 flex-1 items-center gap-1.5 text-[10px] ${sidebarCollapsed ? "md:py-1" : ""}`} role="status" title={status === "online" ? "Inbox connected" : status === "connecting" ? "Connecting inbox…" : "Inbox sync unavailable"}><span aria-hidden="true" className={`size-1.5 shrink-0 rounded-full ${status === "online" ? "bg-emerald-500" : status === "connecting" ? "animate-pulse bg-amber-500 motion-reduce:animate-none" : "bg-muted-foreground"}`} /><span className={sidebarCollapsed ? "truncate md:sr-only" : "truncate"}>{status === "online" ? "Connected" : status === "connecting" ? "Connecting…" : "Offline"}</span></span>
