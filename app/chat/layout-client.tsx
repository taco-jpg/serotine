...
function InboxLayout({ children }: { children: React.ReactNode }) {
  ...
  const createGroup = async (event: React.FormEvent) => {
    ...
    try {
      ...
      setGroupOpen(false); setGroupName(""); setGroupMembers([]); setFilter(""); router.push(`/chat/${id}`)
  ...
  const requestAction = async (conversation: ConversationRecord, accept: boolean) => {
    ...
    try {
      if (accept) { await messaging.acceptRequest(conversation.id); setRequestsOpen(false); router.push(`/chat/${conversation.id}`) }
  ...
  return <div className="flex h-dvh min-h-0 overflow-hidden bg-zinc-950 text-zinc-200">
    <aside aria-label="Inbox" className={`${inConversation ? "hidden md:flex" : "flex"} w-full flex-col border-r border-zinc-800/80 bg-zinc-900/25 md:w-[22rem] md:shrink-0`}>
      ...
      <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain">
        <div className="px-3 pt-3">{selfConversation ? <ConversationRow conversation={selfConversation} selected={pathname === `/chat/${identity.publicKey}`} owner={identity.publicKey} /> : <Link href={`/chat/${identity.publicKey}`} className="flex items-center gap-3 rounded-xl p-3 text-sm font-medium text-indigo-200 hover:bg-zinc-900"><UserRound className="size-5" /> Message yourself</Link>}</div>
