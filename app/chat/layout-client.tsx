...
import { ProfileEditor, ProfileSharingFields } from "@/components/profile/profile-settings"
...
function InboxLayout({ children }: { children: React.ReactNode }) {
  ...
  const displayName = (pub: string) => pub === identity?.publicKey ? nickname || "You" : contacts.find(contact => contact.pub === pub)?.alias || messaging.getProfile(pub).displayName || shortAddress(pub)
  ...
  const [addSharing, setAddSharing] = useState<ProfileField[]>([])
  const [acceptSharing, setAcceptSharing] = useState<Record<string, ProfileField[]>>({})
  useEffect(() => { if (addOpen) setAddSharing([]) }, [addOpen])
  ...
  const addContact = async (event: React.FormEvent) => {
    ...
    try {
      ...
      if (pub !== identity.publicKey) {
        ...
        await messaging.setProfileSharing(pub, addSharing)
      }
  ...
  const requestAction = async (conversation: ConversationRecord, accept: boolean) => {
    ...
    try {
      if (accept) { await messaging.acceptRequest(conversation.id); if (conversation.kind === "direct") await messaging.setProfileSharing(conversation.id, acceptSharing[conversation.id] || []); setRequestsOpen(false); router.push(conversationHref(conversation.id)) }
      else if (conversation.invitation) await messaging.declineGroupInvitation(conversation.id)
  ...
  return <div className="chat-shell flex min-h-0 overflow-hidden bg-background text-foreground">
    <aside id="serotine-sidebar" aria-label="Inbox" data-collapsed={sidebarCollapsed} className={`${inConversation ? "hidden md:flex" : "flex"} min-h-0 min-w-0 w-full flex-col overflow-hidden border-r border-border bg-sidebar text-sidebar-foreground transition-[width] duration-200 ease-out motion-reduce:transition-none md:shrink-0 ${sidebarCollapsed ? "md:w-16" : "md:w-[16.5rem]"}`}>
      <div className={`flex h-15 shrink-0 items-center gap-1 border-b border-border px-3 ${sidebarCollapsed ? "md:justify-center md:px-2" : ""}`}>
        <Link href="/chat" aria-label="Serotine inbox" className={`flex min-h-11 min-w-0 flex-1 items-center rounded-[4px] focus-visible:outline-2 focus-visible:outline-ring ${sidebarCollapsed ? "md:hidden" : ""}`}><AppLogo className="text-[25px]" /></Link>
        <Button variant="ghost" size="icon" className={`size-11 shrink-0 text-muted-foreground md:size-8 ${sidebarCollapsed ? "md:hidden" : ""}`} aria-label="Invite a friend" title="Invite a friend" onClick={() => { setCopied(false); setShareOpen(true) }}><QrCode className="size-4" /></Button>
      ...
      <div className={`min-h-0 flex-1 overflow-y-auto overscroll-contain ${sidebarCollapsed ? "md:hidden" : ""}`}>
        ...
        {(visibleConversations.length > 3 || filter) && <div className="relative mx-3 mb-2"><Search className="pointer-events-none absolute left-2.5 top-1/2 size-3.5 -translate-y-1/2 text-muted-foreground" /><Input aria-label="Filter conversations" placeholder="Filter conversations" value={filter} onChange={event => setFilter(event.target.value)} className="pl-8 md:h-8" /></div>}
        ...
        <nav className="space-y-0.5 px-3 pb-2" aria-label={conversationView === "archived" ? "Archived conversations" : "Conversations"}>
          ...
          <ActivityList items={filteredConversations} scope={`${identity.publicKey}:${conversationView}:${filter}`} className="space-y-1">{conversation => {
      ...
      <div className={`${sidebarCollapsed ? "hidden md:flex" : "hidden"} min-h-0 flex-1 flex-col items-center gap-1 overflow-y-auto overflow-x-hidden overscroll-contain px-2 py-2`}>
        ...
        <nav aria-label={conversationView === "archived" ? "Archived conversations" : "Conversations"} className="flex flex-col items-center gap-1 pt-1">
          <ActivityList items={visibleConversations} scope={`${identity.publicKey}:${conversationView}:compact`} className="space-y-1">{conversation => conversation.kind === "community" ? <CommunityRow collapsed community={conversation} archived={conversation.archived} selected={selectedCommunity === conversation.id} owner={identity.publicKey} displayName={displayName} /> : <ConversationRow collapsed conversation={conversation} selected={selectedConversation === conversation.id} owner={identity.publicKey} displayName={displayName} />}</ActivityList>
        </nav>
    ...
    <Dialog open={!!remove} onOpenChange={open => { if (!open) setRemove(null) }}><DialogContent><DialogHeader><DialogTitle>Remove this contact?</DialogTitle><DialogDescription>This removes {remove?.alias || "the contact"} and ends server retention for this direct relationship. Already downloaded history stays on both devices. Both people must add or accept the contact again before new relay messages can be retained.</DialogDescription></DialogHeader><DialogFooter><Button variant="outline" onClick={() => setRemove(null)}>Cancel</Button><Button variant="destructive" onClick={async () => { if (!remove) return; try { await messaging.removeFriend(remove.pub); setNotice("Contact removed. Server retention ended; downloaded history remains on this device."); setRemove(null) } catch (cause) { setError(errorMessage(cause)) } }}>Remove contact</Button></DialogFooter></DialogContent></Dialog>
  </div>
}
