...
export function LoginForm() {
  ...
  useEffect(() => {
    void loadIdentity().then(identity => setHasIdentity(!!identity)).catch(cause => { setError(cause.message); setRestore(true) }).finally(() => setChecking(false))
  }, [])
  ...
  return <div className="space-y-5">
    ...
    {hasIdentity ? <>
      ...
      {!restore ? <>
        ...
        <div className="space-y-2"><Label htmlFor="backup-password">Backup password</Label><Input id="backup-password" type="password" autoComplete="current-password" value={password} onChange={event => setPassword(event.target.value)} /><p className="text-sm text-zinc-500">Leave blank for an older unencrypted key export.</p></div>
      ...
      <Button variant="ghost" className="w-full" disabled={loading} onClick={() => { setRestore(!restore); setError(null) }}>{restore ? "Create a new identity" : "I have an identity backup"}</Button>
    </>}
  </div>
}
