...
export function PollCreator({ open, onOpenChange, onCreate }: {
  ...
  return <Dialog open={open} onOpenChange={value => { if (!busy) onOpenChange(value) }}><DialogContent className="max-h-[90dvh] overflow-y-auto"><DialogHeader><DialogTitle>Create a poll</DialogTitle><DialogDescription>Everyone in this conversation can choose one answer and change their vote.</DialogDescription></DialogHeader>
    <form className="space-y-4" onSubmit={event => void submit(event)}>
      ...
      {options.every(option => option.trim()) && new Set(options.map(option => option.trim().toLocaleLowerCase())).size !== options.length && <p className="text-sm text-amber-700 dark:text-amber-300">Give each answer different text.</p>}
...
export function GroupSettings({ groupName, members, candidates, canManage, left, ownerPub, myPub, onRename, onAdd, onRemove, onLeave }: {
  ...
  return <div className="space-y-5">
    ...
    <div><h3 className="mb-3 text-sm font-medium">Members · {members.length}</h3><ul className="space-y-2">{members.map(member => <li key={member.pub} className="flex items-center justify-between gap-3 rounded-lg border border-border px-3 py-2"><div className="min-w-0"><p className="truncate text-sm">{member.pub === myPub ? "You" : member.label}</p><p className="text-xs text-muted-foreground">{member.pub === ownerPub ? "Group admin" : "Member"}</p></div>{canManage && !left && member.pub !== myPub && <Button type="button" variant="ghost" size="icon" aria-label={`Remove ${member.label} from group`} disabled={busy} onClick={() => setRemove(member)}><UserMinus className="size-4" /></Button>}</li>)}</ul></div>
    {canManage && !left && <form className="space-y-2" onSubmit={event => { event.preventDefault(); void act(async () => { await onAdd(newPub.trim()); setNewPub("") }) }}><Label htmlFor="group-add-member">Invite a member</Label>
      <QrScanner disabled={busy} onScan={address => {
