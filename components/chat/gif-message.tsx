...
function GiphyCard({ id }: { id: string }) {
  ...
  return <div className="w-96 max-w-full overflow-hidden rounded-lg">
    {load && gif && !error ? <>
      ...
      {configured ? !load ? <Button type="button" variant="outline" size="sm" className="min-h-11" onClick={() => setLoad(true)}>Load GIF</Button> : !error && <p role="status" className="flex items-center gap-2 text-xs"><Loader2 className="size-4 animate-spin" aria-hidden="true" />Loading GIF…</p> : <p className="text-xs text-muted-foreground">Inline GIF viewing hasn’t been enabled on this site yet.</p>}
    ...
    <div className="flex min-w-0 items-center gap-2 text-xs text-muted-foreground">
      ...
      {load && <button type="button" className="ml-auto min-h-11 min-w-11 shrink-0 rounded-md hover:underline focus-visible:outline-2" onClick={() => { setLoad(false); setGif(null); setError("") }}>Hide GIF</button>}
    </div>
  </div>
}
