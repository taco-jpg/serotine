...
import { ListChecks, ArrowDown, AtSign, BarChart3, Bell, Check, ChevronDown, ChevronUp, Copy, ExternalLink, Flag, Hash, Info, Loader2, Megaphone, MoreHorizontal, Pencil, Pin, PinOff, Plus, Reply, RotateCw, Search, Send, Shield, Sparkles, Trash2, X } from "lucide-react"
...
function CommunityTextConversation({ api, community, channel, onReport, onHide }: CommunityConversationProps) {
  ...
  const displayName = useCallback((pub: string) => pub === myPub ? nickname || "You" : names.get(pub) || plugins.getProfile(pub).displayName || shortAddress(pub), [myPub, nickname, names, plugins])
  ...
  return <div ref={chatRoot} className="relative flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden">
    ...
    <div ref={viewport} role="region" aria-label="Channel messages" tabIndex={0} className="min-h-0 min-w-0 flex-1 overflow-x-hidden overflow-y-auto overscroll-contain px-2 py-3 sm:px-5" onScroll={event => { const node = event.currentTarget, wasNear = nearBottom.current; nearBottom.current = node.scrollHeight - node.scrollTop - node.clientHeight < 120; setAwayFromBottom(!nearBottom.current); if (nearBottom.current) { setUnseen(0); if (!wasNear) acknowledgeVisible() } }}>
      ...
      {messages.map((message, index) => {
        ...
        const continuation = inRun(previous, message), showMetadata = !inRun(message, next) || message.editedAt || message.pinned || message.delivery === "pending" || message.delivery !== next?.delivery
        ...
        return <div key={message.id} id={`community-message-${message.id}`} ref={node => { if (node) messageNodes.current.set(message.id, node); else messageNodes.current.delete(message.id) }} data-message-run={continuation ? "continuation" : "start"} className={`${continuation ? "mt-0.5" : "mt-2"} ${activeMatch === message.id || highlightedId === message.id ? "rounded-sm ring-2 ring-primary/60 ring-offset-2 ring-offset-background" : ""}`}>{selectedMessages !== null && canShareMessage(message) && <label className="mb-1 flex min-h-10 cursor-pointer items-center gap-2 text-xs text-muted-foreground"><input type="checkbox" className="size-4 accent-primary" aria-label={`Select message ${message.id}`} checked={selectedMessages.includes(message.id)} disabled={!selectedMessages.includes(message.id) && selectedMessages.length >= MAX_SHARED_MESSAGES} onChange={() => setSelectedMessages(current => toggleSharedSelection(current || [], message.id))} />Select message</label>}
            ...
            <div className={`flex w-full min-w-0 items-start gap-1 ${mine ? "flex-row-reverse" : ""}`}>
              <div className={`min-w-0 max-w-[calc(100%-2rem)] break-words rounded-sm border px-2.5 py-1 text-[15px] leading-[1.4] [overflow-wrap:anywhere] sm:max-w-[min(85%,75ch)] ${mine ? "border-primary/15 bg-message-outgoing text-message-outgoing-foreground" : "border-border bg-message-incoming text-message-incoming-foreground"}`}>
                {message.hidden ? <p className="text-sm italic text-muted-foreground">Message hidden by a moderator.</p> : <>
                  ...
                  {message.shared ? <SharedMessageBundle bundle={message.shared} /> : !!message.content && !message.poll && <RichMessage text={message.content} highlight={query.trim()} mentions={message.mentions} displayName={displayName} />}
