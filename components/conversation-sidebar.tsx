...
export function ConversationRow({ conversation, selected, owner, actions, displayName, collapsed = false }: {
  ...
  const icon = conversation.kind === "group" ? <span className="flex size-8 shrink-0 items-center justify-center rounded-[4px] border border-border bg-background text-primary"><Users className="size-4" /></span>
  ...
  const rowStyle = selected ? "border-primary bg-primary/10" : "border-transparent hover:bg-accent/80"
  ...
  return <div className={`group/row flex items-center gap-0.5 rounded-[4px] border-l-2 transition-colors motion-reduce:transition-none ${rowStyle}`}>
    <Link href={conversationHref(conversation.id)} aria-current={selected ? "page" : undefined} className="flex min-h-12 min-w-0 flex-1 items-center gap-2.5 rounded-[4px] px-2 py-1.5 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-ring">
      {icon}
      <span className="min-w-0 flex-1">
        <span className="flex items-center gap-1.5"><span className={`truncate text-[13px] text-foreground ${conversation.unreadCount ? "font-semibold" : "font-medium"}`}>{name}</span>{conversation.archived && <Archive className="size-3 shrink-0 text-muted-foreground" aria-label="Archived" />}{conversation.notificationMode === "muted" && <BellOff className="size-3 shrink-0 text-muted-foreground" aria-label="Muted" />}<span className="ml-auto shrink-0 font-mono text-[9px] text-muted-foreground">{last ? messageTime(last.timestamp) : ""}</span></span>
        <span className="flex items-center gap-1 text-xs text-muted-foreground">
          ...
          {conversation.unreadCount > 0 && <span aria-label={`${conversation.unreadCount} unread messages`} className="ml-auto flex min-w-5 shrink-0 items-center justify-center rounded-[3px] bg-primary px-1.5 py-0.5 font-mono text-[10px] text-primary-foreground">{conversation.unreadCount > 99 ? "99+" : conversation.unreadCount}</span>}
        </span>
      </span>
    </Link>
    {actions}
