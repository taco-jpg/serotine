...
export function QrCodeCard({ value, title = "Contact QR code", caption = "This QR code contains the full public address. Scan it in Serotine to add the contact.", downloadName = "serotine-contact-qr.png" }: { value: string; title?: string; caption?: string; downloadName?: string }) {
  ...
  return <div className="flex min-w-0 flex-col items-center gap-2">
    ...
    {url && <Button asChild type="button" variant="outline" size="sm"><a href={url} download={downloadName}><Download className="size-4" />Save QR image</a></Button>}
  </div>
}
