...
export async function directSignaling(identity: Identity, action: "direct:signal" | "direct:poll", data: { signal: DirectSignal } | { peers: string[] }): Promise<DirectSignal[]> {
  ...
  const response = await apiFetch("/api/direct", { method: "POST", credentials: "same-origin", mode: "same-origin", cache: "no-store", redirect: "error", referrerPolicy: "strict-origin",
