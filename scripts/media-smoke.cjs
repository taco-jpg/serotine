...
async function main() {
  ...
  try {
    ...
    assert.equal(await history.getByRole('button', { name: 'Load GIF', exact: true }).count(), 0, 'choosing a GIF must not send it')
    await page.getByRole('button', { name: 'Send message', exact: true }).click()
    const loadGif = history.getByRole('button', { name: 'Load GIF', exact: true })
    await loadGif.waitFor()
    const beforeLoad = providerRequests.length
    await page.reload()
    await loadGif.waitFor()
    assert.equal(providerRequests.length, beforeLoad, 'history reload does not contact provider before Load GIF')
    await loadGif.click()
    await history.getByRole('img', { name: 'Synthetic red square', exact: true }).waitFor()
    await history.getByRole('img', { name: 'Synthetic red square', exact: true }).evaluate(img => img.decode())
    ...
    console.log('PASS mocked GIF search failure/retry, explicit staging/send, recipient load consent and provider request privacy')
