...
async function main() {
  ...
  try {
    ...
    await applied(page, 'forest', 'light', '#eaefe7')
    await chooseMode(page, 'Dark')
    await applied(page, 'forest', 'dark', '#0b100d')
    ...
    for (const [name, id, background] of [['Lavender', 'lavender', '#100e13'], ['Rose', 'rose', '#130e10'], ['Monochrome', 'monochrome', '#0e0f0d'], ['Ocean', 'ocean', '#0b1012']]) {
    ...
    await applied(page, 'ocean', 'dark', '#0b1012')
