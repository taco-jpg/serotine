...
async function main() {
  ...
  try {
    ...
    await applied(page, 'forest', 'light', '#e4f5eb')
    await chooseMode(page, 'Dark')
    await applied(page, 'forest', 'dark', '#061a13')
    ...
    for (const [name, id, background] of [['Lavender', 'lavender', '#1c0c30'], ['Rose', 'rose', '#2a0c1b'], ['Monochrome', 'monochrome', '#101010'], ['Ocean', 'ocean', '#071630']]) {
    ...
    await applied(page, 'ocean', 'dark', '#071630')
